...
*   **常见基线模型与神经网络对比模型：** 
    ...
    *   **RandomForest (随机森林回归)**
    *   **XGBoost (XGBoost 回归)**
    *   **MLP (多层感知机回归 - MSE损失):** 一个标准的神经网络模型，使用均方误差(MSE)损失函数，其结构与CAAR部分组件相似，用于对比基于神经网络的非鲁棒方法。
    *   **MLP_Pinball_Median (多层感知机回归 - Pinball损失实现中位数回归):** 与上述MLP结构相同，但使用Pinball损失函数（quantile=0.5）来拟合条件中位数。它作为一种基于神经网络的、以优化中位数目标为导向的鲁棒性方法进行评估，并与CAAR/GAAR在鲁棒性表现上比较。由于传统基于线性规划的`QuantileRegressor`在较大规模数据上可能存在计算效率问题，此MLP实现提供了一个潜在的更快速替代方案。
*   **现有（传统）鲁棒回归模型：** 
    *   **Huber (Huber 回归):** 使用 Huber 损失函数，有效降低异常点的影响，是广泛应用的鲁棒基准。
*   **我们的新方法：**
    *   **CAAR (柯西推断行动回归)**
...
*   **已纳入的核心对比模型:**
    ...
    *   **Huber 回归:** 作为广泛认可的、基于损失函数调整的传统鲁棒方法基准。
    *   **MLP (MSE损失):** 作为与CAAR/GAAR/MLP_Pinball结构相似但采用标准MSE损失的神经网络基线。
    *   **MLP_Pinball_Median (Pinball损失中位数回归):** 作为基于神经网络的、直接优化中位数目标的鲁棒方法。它不仅与CAAR/GAAR在鲁棒性上形成对比，也作为对传统中位数回归（如`sklearn.linear_model.QuantileRegressor`）在计算效率和非线性能力方面的一种探索。
    *   **CAAR 和 GAAR:** 我们提出的核心创新方法，进行重点评估和对比。
*   **暂时排除或在部分实验中未重点分析的模型:**
...
因此，本报告中分析的模型主要围绕 **OLS, RandomForest, XGBoost, Huber, MLP (MSE), MLP_Pinball_Median, GAAR, 和 CAAR** 展开，这个组合能够较好地覆盖不同类型的基准和我们关注的创新点。
...
*   **OLS, Huber:** 使用 scikit-learn 的默认参数，或在 `src/experiments/real_exp.py` 中指定的参数 (例如, `HuberRegressorWrapper(epsilon=1.35)`). (注: Ridge 已被移除)
...
*   **神经网络模型 (CAAR, GAAR, MLP, MLP_Pinball_Median):**
...
| Huber              | 0.544504 | 0.737793 | 0.518557 | 0.383863 |  0.592198 |          0.072193   |
...
*   **Huber表现相对稳健：** Huber (MSE 0.5445, R² 0.5922) 在此次强异常值注入下表现优于其他基线模型（OLS, RandomForest, XGBoost, MLP, GAAR），R²为正，显示了一定的鲁棒性。\n\
...
| Huber              | 0.665403 | 0.813661 | 0.583103 | 0.43814  | 0.501531 |          0.0587961  |
...
*   **XGBoost表现尚可：** XGBoost (R² 0.7827) 的表现略逊于上述模型，但仍然优于OLS和Huber。\n\
*   **传统模型差距明显：** OLS (R² 0.4073) 和 Huber (R² 0.5015) 在X轴异常（强度10.0）下表现不佳，R²值远低于其他神经网络模型和RandomForest。
...
| Huber              |  2980.07 |  54.5577 |  45.0489 | 39.9951 |  0.484006 |         0.0017888   |
...
*   **Huber表现稳健：** Huber回归 (MSE 2980.1, R² 0.4840) 在此场景下也展现了较好的鲁棒性。\n\
...
| Huber              |  17.6741  |  4.16338 |  2.98574 |  2.19848 |  0.784333 |         0.00197694  |
...
*   **Huber表现稳健：** Huber回归 (MSE 17.67, R² 0.7843) 再次证明了其鲁棒性，表现显著优于其他非鲁棒基线模型。\n\
...
| Huber              | 0.0188768 | 0.137275 | 0.0940079 | 0.0614496 |  0.616661  |          0.0845576  |
...
*   **Huber表现稳健：** Huber回归 (MSE 0.0189, R² 0.6167) 同样表现出良好的鲁棒性，优于多数基线模型。\n\
*   **MLP_Pinball_Median表现尚可：** MLP_Pinball_Median (MSE 0.0371, R² 0.2645) 在此数据集上的R²为正，但性能不及CAAR和Huber。\n\
...
| Huber              |  115.084  | 10.7147  |  8.05823 |  6.11855 |  0.584378 |           0.0079804 |
...
*   **Huber表现稳健：** Huber回归 (MSE 115.08, R² 0.5844) 表现出一定的鲁棒性，优于其他非鲁棒基线模型。\n\
...
| Huber              |  20344.8  | 142.619  | 101.616  |  73.2794 |  0.378302  |          0.0375653  |
...
*   **部分基线模型尚可：** Huber (R² 0.3783), XGBoost (R² 0.2149), 和 标准MLP (R² 0.2423) 在此数据集上获得了正的R²值，但与CAAR和MLP_Pinball_Median相比差距显著。GAAR (R² 0.0931) 的表现也一般。\n\
...
| Huber              |  94.5492 |  9.72252 |  7.95387 |  7.19487 |  0.123438 |          0.0514873  |
...
*   **Huber表现一般：** Huber回归 (R² 0.1234) 在此数据集上的表现不如前两个模型，但R²仍为正。\n\
...
*   **传统模型与集成方法的局限性：**
    ...
    *   **Huber：** Huber回归作为一种传统的鲁棒方法，在多数情况下表现稳定，能提供一定的鲁棒性，但通常不如CAAR或MLP_Pinball_Median等更先进的方法。在Communities and Crime数据集上表现相对突出。
    *   **RandomForest和XGBoost：** 这两种强大的集成学习方法在某些数据集上（如RandomForest在Parkinsons Telemonitoring，XGBoost在Bike Sharing）表现出一定的竞争力，但整体而言，它们对Y轴异常的鲁棒性不如专门设计的鲁棒模型（如CAAR, MLP_Pinball_Median），尤其是在异常比例较高或强度较大时（虽然本报告主要展示平均结果，但趋势图通常能反映这一点）。
